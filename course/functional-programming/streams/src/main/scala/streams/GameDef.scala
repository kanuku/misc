...
trait GameDef {
   ...
   * booleans. The function returns `true` for ev[ery position that
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def isLegal: Boolean = isPositive && terrain(b1) && terrain(b2)
    def isPositive: Boolean = b1.x >= 0 && b1.y >= 0 && b2.x >= 0 && b2.y >= 0
